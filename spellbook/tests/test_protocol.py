...
from evaluation.runs import prune_prediction_artifacts, write_run_manifest  # noqa: E402
...
class ProtocolTests(unittest.TestCase):
    def test_fixed_tuple_is_official_and_unique(self):
        ...
        self.assertEqual(len(PREDICTION_EVALUATION_SCENES), 10)
        ...
        self.assertEqual(len(set(bases)), 10)
    ...
    def test_rejects_custom_and_nonselected_and_duplicates(self):
        ...
        with self.assertRaises(ValueError):
            validate_prediction_scenes(["scene0568_01", "0568_01"])
        self.assertEqual(
            validate_prediction_scenes(["0568_01", "scene0575_00"]),
            ["scene0568_01", "scene0575_00"])
        self.assertEqual(
            validate_prediction_scenes(LOCKED, require_complete=True),
            list(LOCKED))
        with self.assertRaises(ValueError):
            validate_prediction_scenes(["scene0568_01"], require_complete=True)
