...
writing ScanNet benchmark instance-segmentation predictions to
/data/scannet/scans/<scene_id>/predictions/<model>/.
...
explicit GPU list, every GPU reported by nvidia-smi is used.
"""
import os
import queue
import subprocess
...
def _output_dir(scene_id, model):
    return os.path.join(SCANS_DIR, scene_id, "predictions", model)


def _run_one(model, scene_id, frames_dir, classes, gpu, label_set="scannet18"):
    """Run one (model, scene) task in a subprocess; returns (model, scene_id, out_dir, elapsed, ok)."""
    out_dir = _output_dir(scene_id, model)
    ...
    args = [MODEL_PYTHON[model], run_script, "--pointcloud", _pointcloud_path(scene_id),
            "--classes", *classes, "--out", out_dir, "--label_set", label_set]
...
def predict(scene_ids, models, classes, gpus=None, label_set="scannet18"):
    """Run predictions for `models` on all `scene_ids`, in parallel over a free-GPU pool.

    `gpus` is a list of physical GPU indices to use; None -> all GPUs from nvidia-smi.
    ...
    if len(gpus) == 1:
        for model, scene_id in tasks:
            results.append(_run_one(model, scene_id, frames_dir_by_scene.get(scene_id), classes, gpus[0], label_set))
        return results
    ...
    def work(task):
        ...
        gpu = free_gpus.get()
        try:
            return _run_one(model, scene_id, frames_dir_by_scene.get(scene_id), classes, gpu, label_set)
        finally:
            free_gpus.put(gpu)
