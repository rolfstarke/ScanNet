...
└── recon/
    ...
    ├── geometry_score.yaml + cad_comparison.png + cad_distance_isometric.png
...
14. **Geometry score (scene9004)**: mean bidirectional nearest-neighbour distance on 10 mm surface voxels vs visible CAD (`mean_bidirectional_distance_cm`, cm, lower better). CAD original orientation is fixed; temporary rigid ICP moves an in-memory recon copy into CAD frame (never saved). Outputs: density overlay `cad_comparison.png` (CAD yellow / recon cyan) and isometric distance-coloured recon `cad_distance_isometric.png` (ceiling height-hide for display only). No PASS/FAIL gate.
